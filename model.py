...
class ModelFactory:
    ...
    def get_feature_dim(self, base_model):
        ...
        """
        if self.cfg.model == "mbn":
            return base_model.last_channel
        elif self.cfg.model.startswith('resnet'):
            return base_model.fc.in_features
        elif self.cfg.model == "efficientnet_b0":
            return base_model.classifier[1].in_features
        elif self.cfg.model == "vgg16":
            return base_model.classifier[6].in_features
        else:
            raise ValueError(f"Unsupported model type: {self.cfg.model}")
    ...
    def initialize_model(self):
        ...
        if self.cfg.multitask:
            ...
            if self.cfg.model in ["mbn", "efficientnet_b0"]:
